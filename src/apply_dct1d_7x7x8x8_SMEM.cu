...
#include "apply_dct1d_7x7x8x8_SMEM.hpp"
#include <time.h>
#include <stdlib.h>
#include <stdio.h>
#include <iostream>
...
{
    const double normalizer = 1.9685019685029528;
    const int block_size = 3136;
    ...
    const int W_stride = Z * Y * X;

    const int relative_X_stride = 1;
    const int relative_Y_stride = SPATIAL_DIM;
    const int relative_Z_stride = SPATIAL_DIM*SPATIAL_DIM;
    const int relative_W_stride = ANGULAR_DIM*SPATIAL_DIM*SPATIAL_DIM;

    int macroblock_x = blockIdx.x * 31;
    int macroblock_y = blockIdx.y * 31;

    /*
        Declarar memória compartilhada 8x8x8x8

        for das 3 posições index i
        for da 3 posições index j

        carrega na memória compartilhada
...
void apply_dct1d_7x7x8x8_SMEM_gpu(double* data,
    ...
    std::cout << "Kernel execution time with memory transfer with smem: " << milliseconds << "\n";
